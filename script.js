...
api_key="0e288389da024d24451d9f02591c55c5",
...
function getWeatherDetails(name, lat, lon, country, state){
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
        ...
        for(let i=1; i<fiveDayForecast.length; i++){
            let date = new Date(fiveDayForecast[i].dt_txt);
            fiveDayForecastCard.innerHTML += `
            <div class="forecast-item">
                    <div class="icon-wrapper">
                        <img src="https://openweathermap.org/img/wn/${fiveDayForecast[i].weather[0].icon}@2x.png" alt="weather icon">
        ...
        alert('Failed to fetch current forecast');
    });
}
...
function getUserCoordinates(){
    navigator.geolocation.getCurrentPosition(position => {
        ...
        if ( error.code === error.PERMISSION_DENIED){
            alert('GEOLOCATION PERMISSION DENIED. Please allow location access and try again.');
        }
    });
}
...
// Auto-hide footer on scroll: hide when scrolling down, show when scrolling up
(function(){
    const footer = document.querySelector('.site-footer');
    if(!footer) return;
    let lastScrollY = window.scrollY || 0;
    let ticking = false;
    const threshold = 10; // minimal scroll (px) to trigger
